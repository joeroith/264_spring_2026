...
You can download this .qmd file from [here](https://github.com/joeroith/264_spring_2026/blob/main/SQL_code/14_SQL.qmd).  Just hit the Download Raw File button.
...
Be sure to set limit so it doesn't run forever. What do you think the 0 and 100 mean?
...
Make (origin, dest) column name less unwieldy by changing column names in resulting data set
...
Be careful with parentheses when using OR.  If you run the code below without parentheses, you'll probably crash R, as it tries to return a ton of records. Do NOT try this at home! 

```{sql, connection = con}
/* takes long time, returns 3194 records  */
SELECT 
  COUNT(*) AS N
FROM flightdata
WHERE Year = 2019 AND (Month = 6 OR DayofMonth = 26)
  AND origin = 'BDL'
```
...
Add MIN as an aggregate function - what is the earliest flight?
...
SELECT 
  Reporting_Airline AS carrier, 
  COUNT(*) AS numFlights,
  MAKETIME(                       -- new function
    IF(LENGTH(MIN(deptime)) = 3, 
      LEFT(MIN(deptime), 1),      -- 1 digit hour if length true
      LEFT(MIN(deptime), 2)       -- 2 digit hour if false
    ), 
    RIGHT(MIN(deptime), 2),       -- minutes always 2 rightmost digits
    0                             -- seconds
...
We've been using LIMIT often to reduce what is returned/printed.  The code below returns the 4th through 7th airports in our list. LIMIT [offset optional], number of rows.
